...
const LandPage = () => {
  ...
  const onGetPermission = async (data) => {
    ...
    try {
      ...
      toast.error(err.reason || "Transaction failed. Note: Only the contract owner can grant permission.");
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        {/* --- MODIFIED: Header with Connect Wallet button --- */}
        <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            {account ? (
              <div className="p-2 bg-green-100 text-green-800 rounded-lg text-sm shadow-sm">
                ✅ Connected: <span className="font-mono">{`${account.substring(0, 6)}...${account.substring(account.length - 4)}`}</span>
              </div>
            ) : (
              <button onClick={handleConnectWallet} className="bg-blue-600 text-white px-6 py-2 rounded-lg shadow hover:bg-blue-700 transition-colors">
                Connect Wallet
              </button>
            )}
          </div>
        ...
        <div className="bg-white rounded-xl shadow-soft overflow-hidden">
          ...
          <div className="p-6">
            {/* Add New Land Tab - YOUR ORIGINAL DETAILED UI */}
                ...
                <form onSubmit={handleNewSubmit(onAddNewLand)} className="space-y-6">
                  <div className="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div><label className="form-label">Land Location</label><input {...registerForm('location', { required: 'Land location is required' })} type="text" className="form-input" placeholder="e.g. District, City" />{newErrors.location && <p className="form-error">{newErrors.location.message}</p>}</div>
            ...
            {/* Transfer Land Tab - YOUR ORIGINAL UI WITH NEW LOGIC */}
            ...
            {/* Building Permission Tab - YOUR ORIGINAL UI WITH NEW LOGIC */}
