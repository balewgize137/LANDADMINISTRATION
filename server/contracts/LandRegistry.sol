...
contract LandRegistry {
    ...
    function getTotalLands() public view returns (uint256) { return totalLands; }
    ...
    function getVerifiedLandsCount() public view returns (uint256) {
        uint256 count = 0;
        for (uint i = 1; i <= totalLands; i++) { if (lands[i].isVerified) { count++; } }
        return count;
    }
    function getAllLands() public view returns (Land[] memory) {
        Land[] memory allLands = new Land[](totalLands);
        for (uint i = 0; i < totalLands; i++) { allLands[i] = lands[i + 1]; }
        return allLands;
    }
}